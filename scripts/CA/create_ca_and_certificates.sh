...
CERT_DIR_PARAM=${1:-"$SCRIPT_DIR/../../nginx/certs"}
...
openssl req -newkey rsa:4096 -sha256 -nodes -config ../cnf/server.cnf -keyout privkey.pem -out server.csr # -subj "/CN=inz.localhost" -addext "subjectAltName=DNS:inz.localhost,DNS:localhost,IP:127.0.0.1,IP:::1"
